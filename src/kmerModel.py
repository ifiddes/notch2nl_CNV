...
def get_normalizing_kmers(normalizing_path):
    """
    Loads a set of normalizing kmers from normalizing_path. These are expected to be one per line
    in a text file. This file would have been generated by finding kmers present in all five paralogs.
    """
    return {x.rstrip() for x in open(normalizing_path)}
...
def get_kmer_counts(graph, normalizing_kmers, kmer_counts_file):
    ...
    for count, seq in count_reader(kmer_counts_file):
        ...
        if seq in normalizing_kmers:
            normalizing += count
    normalizing = 0.2 * normalizing / len(normalizing_kmers)
